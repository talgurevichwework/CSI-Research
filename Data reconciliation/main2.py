...
cw_df = we.get_tbl_query(queries.create_salesforce_closedwon_query_notrunc(time_period, start_date, end_date))
cl_df = we.get_tbl_query(queries.create_salesforce_closedlost_query_notrunc(time_period, start_date, end_date))
re_df = we.get_tbl_query(queries.create_sapi_reuserecords_query_notrunc(time_period, start_date, end_date))
...
comp_df = comp_df[['account_uuid', 'Sf Net Desk Change', 'desks_changed', 'Sf Looker Difference', 'Sf Looker Absolute Difference']]
comp_df = comp_df.rename(index=str, columns={"account_uuid": "Account UUID", "desks_changed": "Looker Net Desk Change"})
